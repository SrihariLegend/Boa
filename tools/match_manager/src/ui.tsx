...
import type {EngineSpec, GameDetail, MatchConfig, MatchSettings, MatchSummary} from "./types.js";
...
export function App(): React.ReactElement {
  ...
  return (
    <Box flexDirection="column">
      ...
      {screen.name === "matches" && <Matches setScreen={setScreen} />}
...
function Matches({setScreen}: {setScreen: (screen: Screen) => void}): React.ReactElement {
  const matches = store.summaries();
  const [selected, setSelected] = useState(0);
  useInput((input, key) => {
    if (key.upArrow) setSelected((n) => Math.max(0, n - 1));
    if (key.downArrow) setSelected((n) => Math.min(Math.max(matches.length - 1, 0), n + 1));
  ...
  return (
    <Box marginTop={1} flexDirection="column">
      ...
      <Text color="gray">n new match  enter details</Text>
      <Box marginTop={1} flexDirection="column">
        {matches.length === 0 && <Text color="gray">No matches yet.</Text>}
        {matches.map((match, index) => <MatchLine key={match.id} match={match} active={index === selected} />)}
      </Box>
    </Box>
  );
}
...
function Detail({id, setScreen, flash}: {id: string; setScreen: (screen: Screen) => void; flash: (kind: "ok" | "error", text: string) => void}): React.ReactElement {
  ...
  useInput((input, key) => {
    ...
    if (key.upArrow) setSelected((n) => Math.max(0, n - 1));
    if (key.downArrow) setSelected((n) => Math.min(Math.max(games.length - 1, 0), n + 1));
    if (key.return && games[selected]) setScreen({name: "replay", id, game: games[selected].index});
    if (input === "s") {
      match.stop();
      flash("ok", `Stopped ${id}`);
    }
    if (input === "d" && match.status !== "running") setConfirmDelete(true);
  ...
  return (
    <Box marginTop={1} flexDirection="column">
      <Text bold>{summary.id}: {engineLabel(summary.white)} vs {engineLabel(summary.black)} <Status status={summary.status} /></Text>
      <Text color="gray">s stop  d delete stopped/finished  enter replay selected game</Text>
      <Box marginTop={1} gap={2}>
      ...
      <ScoreBar wins={r.wins} draws={r.draws} losses={r.losses} />
      <Box marginTop={1} flexDirection="column">
        {games.length === 0 && <Text color="gray">No PGN games yet.</Text>}
        {games.slice(Math.max(0, selected - 12), selected + 13).map((game) => (
          <Text key={game.index} color={game.index === selected ? "yellow" : resultColor(game.result)}>
            {game.index === selected ? ">" : " "} {(game.index + 1).toString().padStart(3)} {clip(game.white, 22)} - {clip(game.black, 22)} {game.result.padEnd(7)} {game.plies} plies {clip(game.termination, 18)}
          </Text>
        ))}
      </Box>
...
function ScoreBar({wins, draws, losses}: {wins: number; draws: number; losses: number}): React.ReactElement {
  const total = Math.max(1, wins + draws + losses);
  const chunk = (n: number, char: string) => char.repeat(Math.round(30 * n / total));
  return <Text>{chunk(wins, "+")}<Text color="gray">{chunk(draws, "=")}</Text><Text color="red">{chunk(losses, "-")}</Text></Text>;
}
...
function Replay({id, game, setScreen, flash}: {id: string; game: number; setScreen: (screen: Screen) => void; flash: (kind: "ok" | "error", text: string) => void}): React.ReactElement {
  ...
  useInput((input, key) => {
    ...
    if (key.leftArrow) setPly((n) => Math.max(0, n - 1));
    if (key.rightArrow) setPly((n) => Math.min(detail.fens.length - 1, n + 1));
    if (key.upArrow) setPly((n) => Math.max(0, n - 10));
    if (key.downArrow) setPly((n) => Math.min(detail.fens.length - 1, n + 10));
    if (input === "0") setPly(0);
  ...
  return (
    <Box marginTop={1} gap={2}>
      <Box flexDirection="column">
        <Text bold>Game {game + 1}: {detail.headers.White} vs {detail.headers.Black} {detail.headers.Result}</Text>
        <Text color="gray">←/→ ply  ↑/↓ 10 plies  0 start  $ end</Text>
        <Board fen={detail.fens[ply]} />
        <Text color="yellow">{ply} / {detail.fens.length - 1}</Text>
      </Box>
      <Box flexDirection="column" width={42}>
        {detail.sans.slice(Math.max(0, ply - 10), ply + 10).map((san, i) => {
          const actual = Math.max(0, ply - 10) + i + 1;
          return <Text key={actual} color={actual === ply ? "yellow" : undefined}>{actual.toString().padStart(3)} {san}</Text>;
        })}
      </Box>
    </Box>
  );
...
function Board({fen}: {fen: string}): React.ReactElement {
      ...
      {rows.map((row, r) => {
        ...
        for (const ch of row) {
          if (/\d/.test(ch)) {
            ...
            cells.push(pieces[ch] ?? " ");
          }
        }
        return (
          <Box key={r}>
            {cells.map((cell, f) => (
              <Text key={`${r}-${f}`} backgroundColor={(r + f) % 2 === 0 ? "rgb(217,190,142)" : "rgb(122,91,62)"} color={cell === cell.toUpperCase() ? "white" : "black"}>
                {` ${cell} `}
              </Text>
            ))}
          </Box>
