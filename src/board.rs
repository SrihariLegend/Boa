...
fn push_castling_chars(s: &mut String, castling: u8) {
    if castling & CR_WHITE_KINGSIDE  != 0 { s.push('K'); }
    if castling & CR_WHITE_QUEENSIDE != 0 { s.push('Q'); }
    if castling & CR_BLACK_KINGSIDE  != 0 { s.push('k'); }
    if castling & CR_BLACK_QUEENSIDE != 0 { s.push('q'); }
}
...
impl Board {
    ...
    fn place_fen_char(&mut self, ch: char, file: &mut i32, rank: i32, z: &Zobrist) {
        ...
        if pt == PieceType::None || rank < 0 || rank > 7 { return; }
    ...
    pub fn from_fen(fen: &str) -> Option<Self> {
        ...
        if board.side == Color::Black { board.hash ^= z.side; }

        // Castling rights
        let cr_str = parts[2];
        if cr_str.contains('K') { board.castling |= CR_WHITE_KINGSIDE; }
        if cr_str.contains('Q') { board.castling |= CR_WHITE_QUEENSIDE; }
        if cr_str.contains('k') { board.castling |= CR_BLACK_KINGSIDE; }
        if cr_str.contains('q') { board.castling |= CR_BLACK_QUEENSIDE; }
        ...
        if parts[3] != "-" {
            if let Some(sq) = sq_from_name(parts[3]) {
                ...
                let adjacent = ((bb(pawn_sq) << 1) & !BB_FILE_A) | ((bb(pawn_sq) >> 1) & !BB_FILE_H);
        ...
        if parts.len() > 4 { board.halfmove = parts[4].parse().unwrap_or(0); }
        if parts.len() > 5 { board.fullmove = parts[5].parse().unwrap_or(1); }
    ...
    pub fn to_fen(&self) -> String {
        ...
        for rank in (0..8).rev() {
            ...
            for file in 0..8u8 {
                ...
                let p = self.sq_piece[sq as usize];
                if p == PIECE_NONE { empty += 1; continue; }
    ...
    pub fn is_attacked_by(&self, sq: Square, attacker_color: Color) -> bool {
        ...
        if pawn_attacks & bb(sq) != 0 { return true; }

        // Knight attacks
        if knight_attacks(sq) & self.pieces[ac][PieceType::Knight as usize] != 0 { return true; }

        // Bishop/Queen diagonal
        let diag_attackers = self.pieces[ac][PieceType::Bishop as usize]
            | self.pieces[ac][PieceType::Queen as usize];
        if bishop_attacks(sq, occ) & diag_attackers != 0 { return true; }

        // Rook/Queen straight
        let straight_attackers = self.pieces[ac][PieceType::Rook as usize]
            | self.pieces[ac][PieceType::Queen as usize];
        if rook_attacks(sq, occ) & straight_attackers != 0 { return true; }

        // King attacks
        if king_attacks(sq) & self.pieces[ac][PieceType::King as usize] != 0 { return true; }
